...
export async function POST(request: Request) {
  ...
  if (usesBlobStorage) {
    ...
    if (process.env.VERCEL) {
      return NextResponse.json({ message: "Connect a Vercel Blob store before uploading media." }, { status: 503 });
    }
  ...
  const mediaType = file.type === "application/pdf" ? "document" : file.type === "image/svg+xml" ? "icon" : file.type.startsWith("video/") ? "video" : "image";
  const media = {
    id: crypto.randomUUID(),
    filename: file.name.replace(/[^\w.\- ]/g, ""),
    url: mediaUrl,
    type: mediaType as "image" | "document" | "icon",
    size: file.size,
